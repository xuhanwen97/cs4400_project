...
select * from stop join stop as stop1 using (train_id)
	where stop.station_id = (select station_id from station where name = 'Metz')
	and stop1.station_id = (select station_id from station where name = 'Aix')
	and stop.departure_time < stop1.arrival_time;

\! echo list the one-change trips from Metz to Aix
select * from
	(
		select stop.station_id, stop.train_id, stop.stop_time, stop2.station_id as station2_id,
			stop2.train_id as train2_id, stop2.stop_time as stop2_time from stop join stop as stop2
		where stop.station_id = (select station_id from station where name = 'Metz'))
	as metz_depart
	join
	(
		select stop.station_id, stop.train_id, stop.stop_time, stop2.station_id as station2_id, stop2.train_id as train2_id,
			stop2.stop_time as stop2_time from stop join stop as stop2
